...
export function PinScreen() {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 backdrop-blur-md px-3 py-4">
      <div className="bg-white/95 border border-white/50 shadow-2xl rounded-3xl p-4 sm:p-8 w-full max-w-[280px] sm:max-w-sm text-center flex flex-col items-center max-h-[90vh] overflow-y-auto">
        <div className="w-10 h-10 sm:w-16 sm:h-16 bg-blue-100 rounded-full flex items-center justify-center mb-2 sm:mb-6 shadow-sm shrink-0">
          <Lock className="w-5 h-5 sm:w-8 sm:h-8 text-blue-600" />
        </div>
        
        <h2 className="text-base sm:text-2xl font-bold text-slate-900 mb-0.5 sm:mb-2">
          {mode === "setup" 
            ? (step === "enter" ? "Create PIN" : "Confirm PIN") 
            : "Unlock Session"}
        </h2>
        <p className="text-slate-500 text-[11px] sm:text-sm mb-3 sm:mb-8 px-1 leading-tight">
          {mode === "setup"
        ...
        {/* PIN Indicators */}
        <div className="flex gap-2.5 sm:gap-4 mb-3.5 sm:mb-8">
          {[0, 1, 2, 3].map(i => (
            <div 
              key={i} 
        ...
        {/* Numpad */}
        <div className="grid grid-cols-3 gap-x-3 gap-y-2 sm:gap-x-6 sm:gap-y-4 mb-3 sm:mb-6">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
            <button
              key={num}
              onClick={() => handleKeyPress(num.toString())}
              disabled={isProcessing}
              className="w-11 h-11 sm:w-16 sm:h-16 rounded-full bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-lg sm:text-2xl font-semibold text-slate-800 transition-colors shadow-sm flex items-center justify-center"
            >
              {num}
            </button>
          ))}
          ...
          <button
            ...
            className="w-11 h-11 sm:w-16 sm:h-16 rounded-full text-slate-600 hover:bg-slate-100 active:bg-slate-200 transition-colors flex items-center justify-center disabled:opacity-50"
          >
        ...
        {mode === "unlock" && (
          <button 
            ...
            className="flex items-center gap-1.5 text-[11px] sm:text-sm font-medium text-slate-500 hover:text-red-600 transition-colors mt-0.5"
          >
